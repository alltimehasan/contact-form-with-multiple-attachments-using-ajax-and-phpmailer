<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(empty($errors) === true) {
            ...
            $msg = "";
            for ($ct = 0; $ct < count($_FILES['fileToUpload']['tmp_name']); $ct++) {

                $uploadfile = tempnam(sys_get_temp_dir(), hash('sha256', $_FILES['fileToUpload']['name'][$ct]));
                $filename = $_FILES['fileToUpload']['name'][$ct];
                
                if (move_uploaded_file($_FILES['fileToUpload']['tmp_name'][$ct], $uploadfile)) {
